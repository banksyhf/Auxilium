...
namespace Auxilium_Server
{
    public partial class Main : Form
    {
        ...
        private delegate void SendNewUserDelegate(Nexus.Client client);
        #endregion
        public Main()
        {
            try
        ...
        private Dictionary<string, int> GetChannels()
        {
        ...
        private void SendUsers(int channel, string[] moveEvent = null)
        {
            ...
            foreach (ListViewItem item in lvUsers.Items)
                if (((Nexus.Client)item.Tag).Channel == channel)
                    connected.Add(((Nexus.Client)item.Tag).Username);
            ...
            foreach (ListViewItem item in lvUsers.Items)
                if (((Nexus.Client)item.Tag).Channel == channel)
                {
                    ...
                    Thread.Sleep(20); //TODO: Fix that the right way.
